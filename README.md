...
When installing ArchLinux, I took a torrent file and downloaded the torrent file. After doing this, I then installed qTorrent, which is a software that helps with taking torrent files and converting them into the actual file, which allows me to get the .iso image file needed to install Arch Linux to my Virtual Machine. On the other hand, if you do not want to go down the torrent route, you can always visit the [download page](https://archlinux.org/download/) and find the latest mirror file to download the .iso image.
